...
# Baca data asli pengguna
df_orig = pd.read_csv("dataset_timnas_siap_upload_1000.csv")
orig_tweets = [t for t in df_orig["tweet"].dropna().tolist() if len(str(t).strip()) > 5]

# Komponen Variasi Tweet
pos_subjects = [
    ...
    "Formasi modern yang dibawa pelatih kepala", "Struktur pembinaan pemain muda oleh coach",
    "Dukungan penuh suporter kepada pelatih timnas", "Progres permainan skuad garuda di era pelatih saat ini"
]
...
# Sisa tweet netral hingga total 1000
...
while len(orig_tweets) + len(new_tweets) < 1000:
    tmpl = random.choice(net_templates)
    t = tmpl.format(
...
# Gabung dan acak
total_1000 = orig_tweets + new_tweets
random.shuffle(total_1000)

df_out = pd.DataFrame({"tweet": total_1000[:1000]})
df_out.to_csv("dataset_timnas_siap_upload_1000.csv", index=False, encoding="utf-8")
print(f"Selesai! Total baris dalam 'dataset_timnas_siap_upload_1000.csv': {len(df_out)}")
